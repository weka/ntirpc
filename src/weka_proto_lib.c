...
int weka_proto_get_info(int slot, weka_proto_key_t ikey, char *data, int datalen)
{
    ...
    if (slot < 0 || slot >= WEKA_PROTO_MAX_SLOTS)
        return -EINVAL;
...
unsigned short weka_proto_get_endpoints_target_version(int slot)
{
    weka_proto_endpoints_info_t pinfo;

    int rc = weka_proto_get_info(slot, WEKA_PROTO_KEY_ENDPOINTS, (char *)&pinfo, sizeof(weka_proto_endpoints_info_t));
    if (!rc)
        return pinfo.target.ver;

    return rc;
}

int weka_proto_set_docache(int slot, int val)
{
	return -ENOTSUP;
}

int weka_proto_inc_subxid(int slot)
{
	return -ENOTSUP;
}
